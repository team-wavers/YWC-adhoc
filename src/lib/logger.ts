...
const logFormat =
    '{{timestamp}} {{title}} {{file}}:{{line}} ({{method}}) {{tid}} {{message}}';
const sqlFormat = '{{timestamp}} {{title}} {{tid}} {{message}}';
const jsonFormat =
    '{ timestamp:{{timestamp}}, tid:{{tid}}, payload:{{message}} }';
...
const preprocess = (data) => {
    ...
    data.tid = `${
        tid.id() ? tid.id() : '00000000-0000-0000-0000-000000000000'
    }`;

    const file =
        data.file?.length > 23 ? data.file.substring(0, 20) + '...' : data.file;
    data.file = file?.padStart(23, ' ');
    // data.line = data.line?.padEnd(3, ' ');
    data.line = String(data.line)?.padEnd(3, ' ');

    const method =
        data.method?.length > 25
            ? data.method.substring(0, 22) + '...'
            : data.method;
...
const logger = {
    ...
    res(code: number, res: any, req: any) {
        ...
        net.log(message);
        log.log(
            `[RES] [${req.method}] ${req.originalUrl}`,
